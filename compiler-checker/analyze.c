...
static void checkNode(TreeNode *t)
{
	switch (t->kind)
	{
		...
		case ReturnStmt:
		{
			...
			if (t->child[0] == NULL && currentScope->func->type != Void) InvalidReturnError(t->lineno);
			// Break
			break;
		}
		...
		case CallExpr:
		{
			...
			// Check Parameters and Arguments Count
			int paramCount = 0;
			int argCount = 0;

			TreeNode *tmp = paramNode;
			while (tmp != NULL) {
				paramCount++;
				tmp = tmp->sibling;
			}
			...
			if (paramNode->type == Void) paramCount = 0;

			if (argCount != paramCount) InvalidFunctionCallError(t->name, t->lineno);
			// If parameter is Void then there are no arguments
			else if (paramNode->type == Void && argNode != NULL) InvalidFunctionCallError(t->name, t->lineno);
			// If parameter is not Void then there are arguments
			else if (paramNode->type != Void && argNode == NULL) InvalidFunctionCallError(t->name, t->lineno);
			// If parameter is not Void and there are arguments
			else {
				while (paramNode != NULL && argNode != NULL) {
					...
					// Check if the parameter is the array
					if (paramNode->type == IntegerArray) {
						if (paramNode->child[0] != NULL && paramNode->child[0]->type != Integer) ArrayIndexingError(t->name, t->lineno);
						if (argNode->child[0] != NULL && argNode->child[0]->type != Integer) ArrayIndexingError(t->name, t->lineno);
					}
